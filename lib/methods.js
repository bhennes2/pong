...
function newGame(player1Id, player2Id, firstServer) {
  return Games.insert({
    player1:      player1Id,
    player2:      player2Id,
    player1Score: 0,
...
Meteor.methods({
  ...
  playAgain(player1, player2, firstServer) {
    check(player1, Object);
    check(player2, Object);
    check(firstServer, String);

    return newGame(player1._id, player2._id, firstServer);
  },

  setFirstServer(game, firstServer) {
    check(game, Object);
  ...
  updateGame(game, player1Score, player2Score, player1, player2) {
    ...
    if (isOver) {
      ...
      Meteor.call("slackTaunt", game._id, winner, loser);
    }
  },
